...
function memberPhoto(id){
  ...
  console.log('check photo:'+ path);
  try {
  	if(fs.existsSync(path)){
		var m = '/members/'+ id +'.png'
		console.log('ok photo:'+ m);
		return m;
	}
  } catch(e){
...
function access(card_uid){
  ...
  //no admin check if it's open 
  if(isOpen(new Date(),openinghours())) {
    //log the access
    appendLog('OK',card_uid,'Åben, adgang til '+ customer.name, customer.stripeid);
    return {status:'OK',message:'Velkommen to Fablab NV '+ customer.name +'!' };
  } else {
    appendLog('KO',card_uid,'Lukket, ingen adgang til '+ customer.name, customer.stripeid);
    return {status:'KO',message:'Sorry '+ customer.name +' we are close'};
  }

  //
  return {status:'KO',message:'Falltrough id '+ card_uid };
}
function isOpen(currentTime,openinghours){

  //start time
  var fromHour = parseInt(openinghours.from_hour);
  var fromMinutes = parseInt(openinghours.from_minute);
  var startTime = new Date();
  startTime.setHours(fromHour,fromMinutes,0);

  //end time
  var toHour = parseInt(openinghours.to_hour);
  var toMinutes = parseInt(openinghours.to_minute);
  var endTime = new Date();
  endTime.setHours(toHour,toMinutes,0);
